...
function Home() {
  ...
  const addRandomPoint = (x, y, z) => {
    setPoints((prevPoints) => ({
      lat: [...prevPoints.lat, x],
      lon: [...prevPoints.lon, y], 
      alt: [...prevPoints.alt, z], 
    }));
  };
  ...
  return (
    <div className="bg-gradient-to-t from-[#23395d] to-[#152238] w-screen h-screen">
      ...
      <div className="grid grid-cols-3 grid-rows-4 gap-4 m-4"> {/* Grid */}
        ...
        <Paraquedas status={latestData.parachute} conn={connected} className=""/>
        <VelD veld={latestData.veld} conn={connected} className=""/>
        <Altttd alt={latestData.alt} conn={connected} className=""/>
